'use client';

import { useEffect } from 'react';
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { useSignInWithGoogle, useUser } from '@/lib/hooks/use-auth';
import { Button } from '@/components/ui/button';
import { player } from '@/lib/config/player';

function authErrorMessage(error: unknown): string {
  const code = (error as { code?: string } | null)?.code;
  if (code === 'auth/unauthorized-domain') {
    return 'Este domínio não está autorizado no Firebase. Adicione este endereço em Authentication → Settings → Authorized domains.';
  }
  if (code) {
    return `Não foi possível entrar (${code}). Tente novamente.`;
  }
  return 'Não foi possível entrar. Tente novamente.';
}

export default function LoginPage() {
  const router = useRouter();
  const { user, redirectError } = useUser();
  const signIn = useSignInWithGoogle();

  useEffect(() => {
    if (user) router.replace('/admin');
  }, [user, router]);

  const error = redirectError ?? (signIn.isError ? signIn.error : null);

  return (
    <div className="container-app py-16 flex justify-center">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <span className="text-4xl">♟️</span>
          <h1 className="font-display text-2xl text-brand-700 dark:text-brand-400 mt-2">{player.title}</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Acesso do administrador</p>
        </div>

        <div className="card p-6">
          <Button
            type="button"
            variant="secondary"
            className="w-full"
            size="lg"
            loading={signIn.isPending}
            onClick={() => signIn.mutate()}
          >
            <svg className="h-5 w-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
            </svg>
            Entrar com Google
          </Button>

          {error != null && (
            <p className="mt-4 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/30 rounded-lg px-3 py-2">
              {authErrorMessage(error)}
            </p>
          )}
        </div>

        <p className="text-center text-xs text-gray-400 mt-4">
          Apenas para o administrador.{' '}
          <Link href="/" className="text-brand-600 dark:text-brand-400 hover:underline">
            Voltar ao início
          </Link>
        </p>
      </div>
    </div>
  );
}
